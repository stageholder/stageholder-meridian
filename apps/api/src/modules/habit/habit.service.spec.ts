...
describe('HabitService', () => {
  ...
  describe('findById', () => {
    ...
    it('should throw NotFoundException when habit belongs to a different workspace', async () => {
      ...
      await expect(service.findById(habit.id, 'ws-1', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('listByWorkspace (paginated)', () => {
    ...
    it('should clamp limit to MAX_LIMIT', async () => {
      ...
      expect(mockRepository.findByWorkspacePaginated).toHaveBeenCalledWith('ws-1', 1, 500);
    });
  });
});
