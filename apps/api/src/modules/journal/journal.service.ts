...
export class JournalService {
  ...
  async create(workspaceId: string, userId: string, dto: CreateJournalDto): Promise<Journal> {
    ...
    this.lightService.awardJournalEntry(userId, workspaceId, result.value.id).catch((err) => this.logger.warn('Failed to award light for journal entry', err));
    return result.value;
  }
  ...
  private async findByIdInternal(id: string, workspaceId: string): Promise<Journal> {
  ...
  async listByWorkspace(workspaceId: string, userId: string, startDate?: string, endDate?: string, page?: number, limit?: number): Promise<PaginatedResult<ReturnType<Journal['toObject']>>> {
    const member = await this.memberService.requireRole(workspaceId, userId, ['owner', 'admin', 'member']);
    const authorId = member.role === 'owner' || member.role === 'admin' ? undefined : userId;
    if (startDate && endDate) {
      const journals = await this.repository.findByDateRange(workspaceId, startDate, endDate, authorId);
    ...
    const { docs, total } = await this.repository.findByWorkspacePaginated(workspaceId, p, l, authorId);
  ...
  async update(id: string, workspaceId: string, userId: string, dto: UpdateJournalDto): Promise<Journal> {
    ...
    if (journal.authorId !== userId && member.role !== 'owner' && member.role !== 'admin') {
