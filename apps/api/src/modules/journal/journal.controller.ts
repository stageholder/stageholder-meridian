...
import { CreateJournalDto as CreateSchema, UpdateJournalDto as UpdateSchema } from './journal.dto';
...
export class JournalController {
  ...
  @Post()
  async create(@Param('workspaceId') workspaceId: string, @CurrentUserId() userId: string, @Body(new ZodValidationPipe(CreateSchema)) dto: CreateJournalDto) {
  ...
  @Patch(':id')
  async update(@Param('workspaceId') workspaceId: string, @Param('id') id: string, @CurrentUserId() userId: string, @Body(new ZodValidationPipe(UpdateSchema)) dto: UpdateJournalDto) {
