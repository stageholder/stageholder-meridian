...
describe("TodoService", () => {
  ...
  describe("create", () => {
    it("should create a todo and save it", async () => {
      ...
      expect(mockLightService.awardTodoCreate).toHaveBeenCalledWith(
        "user-1",
        "ws-1",
        todo.id,
      );
  ...
  describe("findById", () => {
    ...
    it("should throw NotFoundException when todo belongs to a different workspace", async () => {
      ...
      await expect(service.findById(todo.id, "ws-1", "user-1")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("update", () => {
    ...
    it("should award light when status changes to done", async () => {
      ...
      await service.update(todo.id, "ws-1", "user-1", {
        status: "done",
      } as any);
  ...
  describe("addSubtask", () => {
    it("should add a subtask and save", async () => {
      ...
      const result = await service.addSubtask(todo.id, "ws-1", "user-1", {
        title: "Sub 1",
      } as any);
