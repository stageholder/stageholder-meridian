...
export class TodoListRepository {
  ...
  async save(todoList: TodoList): Promise<void> {
    ...
    await this.model.updateOne({ _id: data.id }, { $set: { name: data.name, color: data.color, icon: data.icon, workspace_id: data.workspaceId, is_shared: data.isShared, is_default: data.isDefault, creator_id: data.creatorId } }, { upsert: true });
  }
  ...
  async findDefaultByWorkspace(workspaceId: string): Promise<TodoList | null> {
    const doc = await this.model.findOne({ workspace_id: workspaceId, is_default: true, deleted_at: null }).lean();
    return doc ? this.toDomain(doc) : null;
  }
  ...
  async findByWorkspacePaginated(workspaceId: string, page: number, limit: number): Promise<{ docs: TodoList[]; total: number }> {
    ...
    const docs = await this.model.find({ workspace_id: workspaceId, deleted_at: null }).sort({ is_default: -1, created_at: -1 }).skip((page - 1) * limit).limit(limit).lean();
  ...
  private toDomain(doc: any): TodoList {
    return TodoList.reconstitute({ name: doc.name, color: doc.color, icon: doc.icon, workspaceId: doc.workspace_id, isShared: doc.is_shared, isDefault: doc.is_default ?? false, creatorId: doc.creator_id, createdAt: doc.created_at, updatedAt: doc.updated_at }, doc._id);
  }
}
