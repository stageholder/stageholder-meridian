...
export class WorkspaceService {
  ...
  async findById(id: string, userId?: string): Promise<Workspace> {
    ...
    if (userId)
      await this.memberService.requireRole(id, userId, [
        "owner",
        "admin",
        "member",
        "viewer",
  ...
  async findByShortId(shortId: string, userId: string): Promise<Workspace> {
    ...
    if (!ws) throw new NotFoundException("Workspace not found");
    await this.memberService.requireRole(ws.id, userId, [
      "owner",
      "admin",
      "member",
      "viewer",
  ...
  async delete(id: string, userId: string): Promise<void> {
    const ws = await this.findById(id);
    if (ws.isPersonal)
      throw new ForbiddenException("Personal workspace cannot be deleted");
