...
function makeWorkspace(overrides: Partial<Record<string, unknown>> = {}): Workspace {
...
describe('WorkspaceService', () => {
  ...
  beforeEach(() => {
    ...
    service = new WorkspaceService(mockRepository as any, mockMemberService as any);
  });
  ...
  describe('create', () => {
    it('should create a workspace and add creator as owner', async () => {
      ...
      expect(mockMemberService.addOwner).toHaveBeenCalledWith(ws.id, 'user-1', 'user@test.com');
    });
  });
  ...
  describe('createPersonal', () => {
    it('should return existing personal workspace if one exists', async () => {
      ...
      const result = await service.createPersonal('user-1', 'user@test.com', 'User');
  ...
  describe('findByUser', () => {
    it('should return workspaces the user is a member of', async () => {
      ...
      expect(mockMemberService.getUserMemberships).toHaveBeenCalledWith('user-1');
    });
  ...
  describe('findById', () => {
    ...
    it('should check membership when userId is provided', async () => {
      ...
      expect(mockMemberService.requireRole).toHaveBeenCalledWith(
        ws.id, 'user-1', ['owner', 'admin', 'member', 'viewer'],
      );
    });
  });
  ...
  describe('update', () => {
    ...
    it('should require owner or admin role', async () => {
      ...
      expect(mockMemberService.requireRole).toHaveBeenCalledWith(ws.id, 'user-1', ['owner', 'admin']);
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should throw ForbiddenException for personal workspace', async () => {
      ...
      await expect(service.delete(ws.id, 'user-1')).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('resolve', () => {
    ...
    it('should throw NotFoundException when identifier does not match', async () => {
      ...
      await expect(service.resolve('nonexistent', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });
});
