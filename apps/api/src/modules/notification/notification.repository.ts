...
export class NotificationRepository {
  ...
  async findByRecipientPaginated(recipientId: string, unreadOnly: boolean, page: number, limit: number): Promise<{ docs: Notification[]; total: number }> {
    const filter: any = { recipient_id: recipientId, deleted_at: null };
    if (unreadOnly) filter.read = false;
  ...
  async countUnread(recipientId: string): Promise<number> {
    return this.model.countDocuments({ recipient_id: recipientId, read: false, deleted_at: null });
  }

  async markAllRead(recipientId: string): Promise<void> {
    await this.model.updateMany({ recipient_id: recipientId, read: false, deleted_at: null }, { $set: { read: true, read_at: new Date() } });
  }
