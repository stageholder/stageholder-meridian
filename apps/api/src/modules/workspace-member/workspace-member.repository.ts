...
export class WorkspaceMemberRepository {
  ...
  async findByWorkspaceAndUser(workspaceId: string, userId: string): Promise<WorkspaceMember | null> {
    const doc = await this.model.findOne({ workspace_id: workspaceId, user_id: userId }).lean();
    return doc ? this.toDomain(doc) : null;
  }

  async findByWorkspaceAndEmail(workspaceId: string, email: string): Promise<WorkspaceMember | null> {
    const doc = await this.model.findOne({ workspace_id: workspaceId, email: email.toLowerCase() }).lean();
    return doc ? this.toDomain(doc) : null;
  }

  async findByToken(token: string): Promise<WorkspaceMember | null> {
    const doc = await this.model.findOne({ invitation_token: token }).lean();
    return doc ? this.toDomain(doc) : null;
  }

  async findByUserId(userId: string): Promise<WorkspaceMember[]> {
    const docs = await this.model.find({ user_id: userId, invitation_status: 'accepted' }).lean();
