...
export class WorkspaceMemberRepository {
  constructor(
    ...
    private model: Model<WorkspaceMemberDocument>,
  ) {}
  ...
  async findByWorkspaceAndUser(
    ...
    const doc = await this.model
      .findOne({ workspace_id: workspaceId, user_id: userId, deleted_at: null })
      .lean();
  ...
  async findByToken(token: string): Promise<WorkspaceMember | null> {
    const doc = await this.model
      .findOne({ invitation_token: token, deleted_at: null })
      .lean();
    return doc ? this.toDomain(doc) : null;
  }
  ...
  async findByWorkspacePaginated(
    ...
    page: number,
    limit: number,
  ): Promise<{ docs: WorkspaceMember[]; total: number }> {
    ...
    return WorkspaceMember.reconstitute(
      {
        ...
        invitationStatus: doc.invitation_status as InvitationStatus,
        invitationToken: doc.invitation_token,
