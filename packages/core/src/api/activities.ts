...
export function createActivitiesApi(client: AxiosInstance, getWorkspaceId: () => string) {
  const wp = (path: string) => workspacePath(getWorkspaceId(), path);

  return {
    list: async (params?: { page?: number; limit?: number }): Promise<{ data: Activity[]; total: number; page: number; limit: number }> => {
